Ultimate('UltimateAggregateObserver').extends({
	...
	exec: function(agg, model, fk, returnOneRow, parentIds) {
		...
		if(group.result.$undefined) {
			var aggg = _.clone(agg);
			delete aggg.model;
			console.log('OBSERVER', aggg, fk, selector, group);
		}
	...
	observe: function(agg) {
		...
		this.observer = this.cursor(agg).observeChanges({
			added: function(id, doc) {
				if(!initializing) this.throttleUpdate(agg, doc, 'added');
			}.bind(this),
			changed: function(id, doc) {
				this.throttleUpdate(agg, doc, 'changed');
			}.bind(this)
		});
	...
	throttleUpdate: function(agg, doc, type) {
		...
		this.throttleCall(key, function() {
			this.update(agg, this.fk ? doc[this.fk] : null); //exec just an aggregate call filtered by id
		}, 500);
	},
	throttleKey: function(type, doc) {
		return this.id + '_' + type + '_' + (this.fk ? doc[this.fk] : 'collection')
	},
