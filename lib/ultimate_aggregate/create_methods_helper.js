...
if(Meteor.isServer) {
	Meteor.methods({
		execAggregateAsync: function(name, modelName, id, groupModelClassName, groupOptions) {
			...
			if(id) return Model.findOne(id)[name](null, true); 
			else if(groupModelClassName) return Model.groupBy(groupModelClassName, groupOptions)[name](null, null, null, true); 
			else return Model[name](null, null, null, true);
		}
	});
}