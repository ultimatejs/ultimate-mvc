Ultimate('CreateAggregateMethodsHelper').extends({
	exec: function(callback) {	
		...
		else if(Meteor.isClient && callback) this.execAggregateAsync(function(res) {
			callback.call(this.context, this.format(res));
		}.bind(this));
		else return this.format(this.findAggregateResult());
	},
	format: function(res) {
		...
		this.context.___useFormatter = null;
