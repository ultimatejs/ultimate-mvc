Ultimate('UltimateAggregateBehavior').extends(UltimateBehavior, {}, {	
	...
	_createAggregateMethod: function(relConfig, name) {
		...
		if(this.owner().isAbstract()) return;
		Meteor.startup(function() {
			this.createAggregateObservers(relConfig, name);
		}.bind(this));
	},
	...
	createAggregateObservers: function(relConfig, name) {
		if(Meteor.isClient) return;
		
		var rel = UltimateUtilities.extractConfig(relConfig, this.ownerPrototype()),
			Model = UltimateUtilities.classFrom(rel.model);
		
		console.log('HELP', rel, this.owner().className);
		
		var agg = UltimateUtilities.extractConfig(Model.prototype.aggregates[name], Model);
			
		agg.aggregate_name = name;
		var aggPub = new UltimateAggregateRelationsPublisher(this, agg, rel);
		aggPub.linkParent(this.owner());
		aggPub.start();
		
		if(true) {
			this.setInterval(function() {
				var agg = UltimateUtilities.extractConfig(Model.prototype.aggregates[name], Model);
				aggPub._agg.selector = agg.selector;
				console.log('INTERVAL MINUTE');
				aggPub.start();
			}, 10000, true);
		}
	},
