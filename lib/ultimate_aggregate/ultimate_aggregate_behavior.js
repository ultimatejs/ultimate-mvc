Ultimate('UltimateAggregateBehavior').extends(UltimateBehavior, {}, {	
	...
	_addAggregateMethods: function(methods) {
		if(methods.aggregates) {
			...
			//we have to re-assign aggregates so parent aggregates stay inherited, rather than overwritten--so child classes get them too
			this.ownerPrototype().aggregates = _.extend({}, this.ownerPrototype().parent.aggregates, this.ownerPrototype().aggregates, methods.aggregates);
		}
		
		if(methods.relations) {
			this.createAggregateMethods(methods.relations);
			
			//we have to re-assign relations so parent relations stay inherited, rather than overwritten--so child classes get them too
			this.ownerPrototype().relations = _.extend({}, this.ownerPrototype().parent.relations, this.ownerPrototype().relations, methods.relations);
		}
	},
	...
	createAggregateMethods: function(relations) {
		_.each(relations, function(rel, name) {
			...
			if(/relation(.{1,5})aggregate/.test(relString)) { //for aggregates defined as relations
				if(/operator:/.test(relString)) this._createRelationAggregateMethod(rel, name); //if no operator, its a pass-through using aggregates on collection model, so don't observer and add methods
			}
	...
	createAggregateObserver: function(name, rel, relationOnly, classAggregateConfig) {
		...
		Meteor.startup(function() {
			if(relationOnly) {
				...
				classAggConfig = UltimateUtilities.extractConfig(classAggregateConfig, this.ownerPrototype()); //for class level aggregates
