Ultimate('UltimateSetupAggregate').extends({	
	...
	onChildStartup: function() {
		this.addAggregateMethods();
		this.createAggregateBasicClassMethods();
	},

	addAggregateMethods: function() {
		this.createAggregateClassMethods(this.aggregates);
		this.createAggregateMethods(this.relations);
	},
	...
	_createAggregateClassMethod: function(aggConfig, name) {
		...
		this.class[name] = function(field, selector, callback, isAsyncFromClient) {	
			callback = _.callbackFromArguments(arguments);

			if(aggConfig) var agg = UltimateUtilities.extractConfig(aggConfig, self)
			else {
				if(name == 'count') {
		...
		if(!/^(sum|avg|count|min|max)$/.test(name)) {
			this.createAggregateObserver(name, {model: this.class}, false, aggConfig);
		}
	},
	_prepareBasicAggregate: function(operator, field, selector) {
		...
		selector = selector || {}
		selector.className = this.className;
	...
	_createAggregateMethod: function(rel, name) {
		this[name+'Agg'] = function(callback, isAsyncFromClient) {	
	...
	createAggregateObserver: function(name, rel, relationOnly, classAggregateConfig) {
		...
		Meteor.startup(function() {
			if(relationOnly) {
				...
				var relConfig = UltimateUtilities.extractConfig(rel, this); //get rel, not agg, config
				...
				var classAggConfig = UltimateUtilities.extractConfig(classAggregateConfig, this); //for class level aggregates
	...
	_createAggregateObserver: function(name, rel) {
		var rel  = UltimateUtilities.extractConfig(rel, this),
			...
			observer = new ObserverClass(rel, agg, this.class);
	...
	_createAggregateObserverRelationOnly: function(name, rel) {
		...
		var observer = new UltimateAggregateRelationObserver(rel, agg, this.class);
		observer.start();
	},
