Ultimate('UltimateAggregatePublisher').extends({
	...
	start: function() {
		...
		//console.log("PREPARE SELECTOR", this._agg.aggregate_name, this._id, agg.selector.agent_id);
		...
		if(Meteor.isServer) { 
			//if(this.isNoObserversForType()) {
				console.log('AGGREGATE PUBLISHER TAKING OVER', this._agg.aggregate_name, this._id);
				this.update(agg);
				this.observe(agg);
			//}
			//else if(this.isActiveObserverForType()) this.observe(agg); //for when cursor change triggers start again
	...
	exec: function(agg, model, fk, returnOneRow, parentIds) {
		...
		//console.log('PIPELINE', selector, group);
	...
	observe: function(agg) {
		...
		this.observer = this.cursor(agg).observe({
			added: function(doc) {
				if(!initializing) {
					console.log('ADDED', agg.aggregate_name, this.fk, doc);
					this.update(agg, this.fk ? doc[this.fk] : null); //exec just an aggregate call filtered by this doc._id
				}
			}.bind(this),
			changed: function(doc) {
				console.log('CHANGED', agg.aggregate_name, this.fk, doc);
				this.update(agg, this.fk ? doc[this.fk] : null); //exec just an aggregate call filtered by this doc._id
			}.bind(this)
		});
