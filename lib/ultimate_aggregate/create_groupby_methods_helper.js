	...
	_combineModelsAndAggregates: function(aggRows, groupModel, options) {
		var groupsObj = {}, 
			ids = aggRows.map(function(group) {
				groupsObj[group.fk || group._id] = group.result; //i totally forget why it might be one or the other
				return group.fk || group._id; //I think exec uses fk and findAggregateResults id
			}),
