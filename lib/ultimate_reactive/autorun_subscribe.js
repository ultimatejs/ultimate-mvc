UltimateReactive.extend({
	...
	ready: function(name) {
		if(!this.___dep_readyDep) this.depend('readyDep');
		...
		return ready || (_.isEmpty(this._subscriptions) && !this.isUltimateDatatableComponent); //ready if no subscriptions, but not for datatables
	}
});