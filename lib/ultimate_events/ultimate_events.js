Ultimate('UltimateEvents').extends({
	...
	_toggleCurrentEvent: function(e) {
		if(this.ultimateComponent) {
			this.ultimateComponent._trueSelf().currentEvent = e || null; //UltimateDatatable uses current e to get rowData
		}
	},
	...
	__getContext: function(context, objString) {
		var uc = this.ultimateComponent._trueSelf();

		if(uc && uc.isComponentModel) { //called from UltimateComponentModel
			if(!objString) context = context;
			else if(objString == 'this') context = uc;
			else context = this._getObj(uc, objString);
		}
		else if(uc) { //called from UltimateComponent
