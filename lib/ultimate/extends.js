_.extend(Ultimate, {
  extends: function (Parent, methods, staticMethods, httpInstanceMethods, httpStaticMethods, clientInstanceMethods, clientStaticMethods, serverInstanceMethods, serverStaticMethods) {
    ...
    if(Class.className != 'UltimateClone') {
		...
		if(typeof ULTIMATE_IS_PACKAGE !== 'undefined' && ULTIMATE_IS_PACKAGE) return Class; //allow packages to export to global scope if desired (see plugin.js for how Build Plugin handles scope issues)
  ...
  babelInherits: function(className, Child, Parent) {
  	...
  	var staticMethods = _.babelExtend({}, Child);
		...
		return Ultimate(className).extends(Parent, methods, staticMethods);
  }
});