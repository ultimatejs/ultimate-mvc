_.extend(Ultimate, {
  extends: function (Parent, methods, staticMethods, httpMethods, staticHttpMethods, clientMethods, clientStaticMethods, serverMethods, serverStaticMethods) {
    if(arguments.length === 0 || !Parent.isUltimate) {
			serverStaticMethods = serverMethods;
      serverMethods = clientStaticMethods;
      clientStaticMethods = clientMethods;
      clientMethods = staticHttpMethods;
      staticHttpMethods = httpMethods;
    ...
    if(staticMethods) this.addMethods(Class, staticMethods);
		...
		if(Meteor.isClient) {
			if(clientMethods) this.addMethods(Class.prototype, clientMethods);
			if(clientStaticMethods) this.addMethods(Class, clientStaticMethods);
		}
		else {
			if(serverMethods) this.addMethods(Class.prototype, serverMethods);
