_.extend(Ultimate, {
  addMethods: function (proto, methods, isClassCreation) {
		...
		info.configNames = this._concatProps(methods.config || proto.config, info.configNames); //append mixin configNames after `methods.configNames`
	...
	combineConfigs: function(proto, methodsGroups, configNames) {
		...
		_.each(methodsGroups, function(group) {
			...
			_.each(configNames, function(name) { 																			//5. COMBINE PREVIOUSLY ADDED CONFIGS
				if(group[name] || proto[name]) {
					configs[name] = this._extract(group[name], proto);

					if(_.isArray(configs[name]) || _.isArray(proto[name])) {																	//6b. CONCAT CONFIG ARRAY, OLDEST TO NEWEST
					...
					else if(_.isPureObject(configs[name]) || _.isPureObject(proto[name])) {												//6c. EXTEND CONFIG OBJECT
