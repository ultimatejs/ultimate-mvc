_.extend(Ultimate, {
	combineFormConfigObjects: function(proto, methods) {
		...
		if(proto.errorMessages && methods.errorMessages) methods.errorMessages =_.extend(proto.errorMessages, methods.errorMessages);
	},
  setupForm: function() {
    if(this.methods.schema) this.schema();
    if(this.methods.forms) this.forms();
		if(this.methods.errorMessages) this.errorMessages();
  },
	...
	schema: function() {
		...
		proto._schema = new SimpleSchema(schema); //inherited or mixed in schemas will have been combined already, and therefore be combined here
	},
	errorMessages: function() {
		var messages = this.methods.errorMessages;
