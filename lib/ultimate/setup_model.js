_.extend(Ultimate, {
	...
	_overrideInsertMethod: function(collection, className) {
		...
		collection.upsert = function(selector, mutator, options, callback) {
			if(!mutator.className) {
				var isModifier = _.objectContains(mutator, '$set', '$setOnInsert', '$inc', '$rename', '$min', '$max', '$mul', '$unset', '$currentDate');
				if(!isModifier) {
				...
				if(!mutator.$set) mutator.$set = {};
				_.extend(mutator.$set, {className: className});
			}
	...
	createClassMethods: function(Class) {
		...
		Class.upsert = function(selector, mutator) {
			...
			if(!mutator.$set) mutator.$set = {};
			_.extend(mutator.$set, {className: Class.className});
	...
	onBeforeInsert: function() {
		...
		var newFunc = function(userId, doc) {
			...
			doc._id = Random.id(); //gotta assign an ID so meteor-collection-hooks doesn't sign a Mongo.objectID, which isnt compatible
		};
