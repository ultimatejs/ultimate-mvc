_.extend(Ultimate, {
  	setupModel: function(methods) {
  		this.createFindMethods(this.class);
		...
		if(!collectionName) {
			if(this.parent.className == 'UltimateUser') {
				...
				this.class.collection = this.parent.prototype.collection; //class .collection already inherited on class is broken by UltimateClone.deepClone
		...
		if(this.parent.collectName != this.class.collectionName)
	...
	_overrideInsertMethod: function(collection, className) {
		...
		collection.upsert = function(selector, modifier, options, callback) {
			modifier.className = className;
			return oldUpsert.call(selector, modifier, options, callback);
		};
	},
	createFindMethods: function(Class) {
