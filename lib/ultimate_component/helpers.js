UltimateComponentParent.extend({
	_deniedRegex: /^(colors|model|tableColumns|onCreated|onRendered|onDestroyed|ar|sub|subLimit|autoruns|subscriptions|limitSubscriptions|onReady|onStop|includes)(\s|$)/,
	...
	_getSpecialHelpers: function() {
		...
		_.each(specialHelpers, function(name) {
			allSpecialHelpers[name] = function() {
				...
				return self[name].apply(self._trueSelf(), args);
			};
		});
