...
.extendStatic({
	...
	current: function() {
		...
		if(!app) throw new Meteor.Error('environment-not-found', 'No environment was found. If this is the client check the client config class. If this is the server check the server config class. Then make sure the host you are currently using is in one of the environment arrays.');
		return app;
	}
})
.extendClientStatic({
...
.extendServerStatic({
	...
	insertApps: function(dontAddApps) {
		...
		if(dontAddApps || this.find().count() > 0) return;
