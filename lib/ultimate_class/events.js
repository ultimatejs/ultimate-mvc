UltimateClass.extendBoth({
  ...
  emitSkipMethod: function(eventName) {
    var args = _.toArray(arguments),
      listeners = this.getListeners(eventName, true), //the only thing different between this method and regular emit() is the onMethod is skipped by passing true here
      behaviors = this.getBehaviors(),
      callNext;
  ...
  getListeners: function(eventName, skipMethod) {
    var onMethod = !skipMethod ? this._onMethod(eventName) : null;

    if(this._listeners) {
      if(!this._listeners[eventName]) return onMethod ? [onMethod] : null;
      else return this.lazyListeners(eventName, skipMethod);
    }
    else return onMethod ? [onMethod] : null;
  },
  lazyListeners: function(eventName, skipMethod) {
    var listeners = this._addedListeners(eventName),
      method = this._onMethod(eventName); //get onSomeMethod attached to class

    return method && !skipMethod ? [method].concat(listeners) : listeners; //prepend onSomeMethod if existent
  },
