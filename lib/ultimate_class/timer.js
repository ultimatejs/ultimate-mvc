UltimateClass.extendBoth({
	...
	throttle: function(key, fn, threshold) {
	  threshhold = threshold || 250;
	  ...
	  return function () {
	    ...
	    if(last.call && now < last.call + threshhold) {
	      ...
	      last.timer = this.setTimeout(function () {
	        last.call = now;
	        fn.apply(this, args);
					this.cleanupThrottle(key, last, threshold);
	      }, threshhold);
	...
	throttleCall: function(key, fn, threshold) {
	  threshhold = threshold || 250;
	...
	cleanupThrottle: function(key, last, threshold) {
		this.setTimeout(function() {
			if(+new Date >= last.call + threshold) delete this._throttleKeys[key];
		}, threshold);
	},
