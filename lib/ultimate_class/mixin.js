_.extend(UltimateClass, {
	...
	_mixinIsStrict: function(Mixin) {
		return Mixin.prototype.hasOwnProperty('strict');
	},
	...
	attachInstanceMethodsStrict: function(mixin) {
		...
		if(this.className == 'UltimateComponentParent') console.log('YO', this.className, mixin.className)
	...
	mixinEventHandlers: function(Mixin, isStrict) {
		if(this.isAbstract() && !_.isEmpty(Mixin.prototype.mixinTo)) return; //event handlers only operate on concerte classes when using `mixinTo`
