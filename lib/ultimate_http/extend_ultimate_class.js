...
UltimateClass.extend({
	...
	transformServer: function(clientObj) {
	    ...
	    for(var prop in obj) {
    		if(obj[prop] && obj[prop].className)  { //instance extended from our classes
					...
					if(typeof obj[prop] == "object" && !_.isDate(obj[prop]) && !_.isArray(obj[prop])) serverObj[prop] = iterate(obj[prop], {}); //standard objects
	...
	check: function() {
		//not currently working. need to match up with simple schema's automatic way of checking
		return
