Ultimate('UltimateUtilities').extends({}, {
	...
	config: function(config, userId, propNames) {
		if(propNames) {
		...
		config = _.mapObject(config, function(val, key) {
			return _.isFunction(val) ? val.call(config, Ultimate.userId(userId)) : val;
		});
	...
	isAllowed: function(config, context, userId, type, name) {
		...
		if(_.isFunction(config.isAllowed) && !config.isAllowed.call(context, userId)) isAllowed = false;
