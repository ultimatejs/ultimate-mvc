Ultimate('UltimateRelationsPublisherFactory').extends({
	construct: function(publisher, subName, ModelClass, aggregates, cachedIdsByCollection) {
	...
	_handleRelation: function(parent, options, name) {
		...
		//SECURITY CHECK
		if(rel === false) return;
		console.log('HELP', name, parent.modelClass.className, rel, rp);
	...
	_relationFromName: function(name, parent, options) {
		...
		//if(!rel) rel = options; //an entire object was provided instead of referencing a relation definition; REMOVED SINCE CLIENT CANT PROVIDE SELECTORS
		...
		//allow for client-provided selectors, but only used in combination with server selectors via $and:
		if(options.selector) rel.selector = {$and: [rel.selector || {}, options.selector]};
		...
		return _.extend(rel, options); //subscribe-provided with options will combine with the relation defined on rel
	},
