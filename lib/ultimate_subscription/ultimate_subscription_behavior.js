Ultimate('UltimateSubscriptionBehavior').extends(UltimateBehavior, {}, {
	...
	_addSubscriptions: function(methods) {
		...
		if(subscriptions) {
			...
			//we have to re-assign subscriptions so parent subscriptions stay inherited, rather than overwritten--so child classes get them too
			this.ownerPrototype().subscriptions = _.extend({}, this.ownerPrototype().parent.subscriptions, this.ownerPrototype().subscriptions, subscriptions);	
		}
	},
