UltimateClone = {
	...
	deepExtendOwnUltimate: function(Child, Parent, func) {
		return this.deepExtendOwn(Child, Parent, function(v, prop) {
			return !Ultimate.usesReservedWord(prop) && (!func || func(v, prop));
		}, 1);
	},
	deepExtendUltimate: function(Child, Parent, func) {
		return this.deepExtend(Child, Parent, function(v, prop) {
			return !Ultimate.usesReservedWord(prop) && (!func || func(v, prop));
		}, 1);
	},
	...
	deepCloneUltimateOnlyProps: function(obj) {
		return this.deepCloneOwn(obj, function(v, prop) {
			var perms = obj.getPrototype().permissions;
			return !Ultimate.usesReservedWord(prop) && !_.isFunction(v) && !_.contains(perms && perms.deniedHttpProps, prop);
		}, 1);
	}
};