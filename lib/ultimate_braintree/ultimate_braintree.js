UltimateBraintree = Ultimate('UltimateBraintree').extends({ //will be moved to package
  ...
  //privateKey: 
	...
	onStartup: function() {
		if(Meteor.isClient) {
			//braintree won't load till after, but here's where it would be assigned instead of in this.tokenizeCard() below
			//this._braintree = braintree; 
			
			this.loadBraintreeJs(this.assignClientToken.bind(this));
		}
		else {
			this._braintree = Meteor.npmRequire('braintree');
			this.connect();
		}
	},
	loadBraintreeJs: function(calback) {
		$.ajax({
