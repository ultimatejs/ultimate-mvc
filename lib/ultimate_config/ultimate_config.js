UltimateConfig = Ultimate('UltimateConfig').extends(UltimateFacade, {
	abstract: true,
	isConfig: true,
	...
	envId: function() {
		return this.isLocalDevelopment() && !this.forceProduction ? 'auto' : this.rootUrl(); //triggers dev env on urls: localhost||127.0.0.1||192.168.etc
	},
	...
	rootUrl: function() {
		return Ultimate.absoluteUrl().stripTrailingSlash();
	},
	...
	absoluteUrl: function(path) {
		...
		return Ultimate.absoluteUrl(path);
	},
	isLocalDevelopment: function() {
		return this.rootUrl().indexOf('localhost') > -1 || this.rootUrl().indexOf('127.0.0.1') > -1 || this.rootUrl().indexOf('192.168') > -1 || this.rootUrl().indexOf(this.localDevUrl) > -1;
	},
	host: function() {
		return Ultimate.absoluteUrl().stripTrailingSlash().replace('http://', '').replace('https://', '');
	},
	...
	absoluteUrl: function(path) {
		return this.prototype.absoluteUrl(path);
	},
	isProduction: function() {
		return this.prototype.isProduction();
	},
	isStaging: function() {
		return this.prototype.isStaging();
	},
	isDevelopment: function() {
