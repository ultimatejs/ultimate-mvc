UltimateModel.extend({
  ...
  unshift: function(att, val, cb) {
	...
	unset: function(att, cb) {
		var obj = {}
		obj[att] = '';
		
    if(Meteor.isClient || cb) {
	    this.db().update(this._id, {$unset: obj}, this.refreshFind.bind(this, cb));
		}
		else {
			this.db().update(this._id, {$unset: obj});
			this.refreshFind();
		}
	...
	pick: function(fields) {
		var fields = _.isArray(fields) ? fields : _.toArray(arguments),
	...
	generateSlug: function(key, slugKey) {
		return this[slugKey || 'slug'] = this[key].toLowerCase().replace(/[^\w ]+/g,'').replace(/ +/g,'-');
	}
});