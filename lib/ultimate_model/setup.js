Ultimate('UltimateSetupModel').extends({
	...
	config: ['permissions', 'subscriptions', 'relations', 'aggregates', 'startData', 'sluggify'],
	...
	onAddMethods: function(methods) {
		...
		this.callParent('onAddMethods', methods);
	},
	...
	setupModel: function(methods) {
  	this.createClassMethods(this.class);
		...
		if(!collectionName) {
			...
			else if(this.parent.collection) {
				this.class.collection =  methods.collection = this.parent.collection; //class .collection already inherited on class is broken by UltimateClone.deepClone
...
/**
	setupHooks: function(methods, proto) {
		this.hookMethods = methods; //methods may come from methods map or parent
