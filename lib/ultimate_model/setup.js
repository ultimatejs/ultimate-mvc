Ultimate('UltimateSetupModel').extends({
	...
	config: ['permissions', 'subscriptions', 'relations', 'aggregates', 'startData', 'sluggify', 'collectionName', 'tableName'],
	...
	setupModel: function(methods) {
		...
		var collectionName = methods.collectionName,
		...
		if(_.isPureObject(collectionName)) { 								//A. already-created Collection object provided (not recommended)
			...
			if(!collectionName && !tableName) { 							//C. collection automatically generated
				if(this.parent.collection) { 										//C1.collection inherited
					...
					collectionName = this.parent.___collectionName;
					collectionInherited = true;
				} 
		...
		this.createClassMethods(this.class, methods.sluggify);
	},
	...
	_assignCollection: function(collection, methods, collectionName) {
		...
		this.collection = this.class.collection = collection; 
	...
	_overrideInsertMethod: function(collection, className, collectionInherited, sluggify) {
		...
		collection.update = function(selector, modifier, options, callback) {
			...
			if(!self._hasOperator(modifier) && !_.isEmpty(modifier)) { //no point doing anything if there is there
			...
			else if(!_.isEmpty(modifier.$set) || !modifier.hasOwnProperty('$set')) { //$set isnt empty or some other operator is being used
	...
	createClassMethods: function(Class, sluggify) {
		var self = this;
