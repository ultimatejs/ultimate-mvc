  ...
  * @param {Function} [callback] - (Client Only) callback to be called with `error` and `result` parameters; same as calls to `collection.insert/update()`
	...
	*/
	save: function(attVals, cb) {
		...
		if(cb || Meteor.isClient) {
			callback = function(error, result) {
					if(!error) this.emit('afterSave', attVals);
					if(_.isFunction(cb)) cb.call(this, error, result);
				}.bind(this);				
		}
		
		var ret = this._upsert(attributes, callback);
		
		if(Meteor.isServer && !cb) this.emit('afterSave', attVals);
	...
	update: function(attributes, cb) {
		...
		if(_.size(attributes) === 1 && attributes.className) { //dont hit db if nothing to send it
			if(_.isFunction(cb)) cb.call(this, new Error('No biggie, nothing to save here.'))
			return this; //degrade silently
		}
