	...
	_prepareCustomFunc: function(customFunc, proto, schemaField, schema) {
		(function(customFunc) {
			schemaField.custom = function() {
				...
				for(var key in schema) model[key] = owner.field(key).value;
		
				var errorType = customFunc.apply(model, [owner]);
	...
	_prepareAutoValue: function(func, proto, schemaField, schema) {
		(function(customFunc) {
			schemaField.custom = function() {
				var model = new proto.class;
				for(var key in schema) model[key] = owner.field(key).value;
				return customFunc.apply(model, [owner]);
			};
		})(customFunc);
	},
    ...
    (function(onSubmit) {
	    ...
	    AutoForm.addHooks([name], {	
	      ...
	      onSubmit: function(data, modifier, beforeDoc) {	
					...
					proto.__applyOnSubmit(onSubmit, model, owner, keys);	
