UltimateDatatableComponent = Ultimate('UltimateDatatableComponent').extends(Meteor.isClient ? UltimateComponent : UltimateClass, {
	abstract: true,
	...
	onCreated: function() { //CALLED ON CLIENT ONLY
		...
		var uc = Ultimate.components[this.className]; //instantiated class from UltimateComponent.prototype.onChildClassCreated
		uc.datatable(); //needs to run all methods off instantiated class
	},

	onChildStartup: function() { //CALLED ON SERVER ONLY
		if(Meteor.isClient || this._tabular) return;
	...
	getOptions: function() {
		...
		var options = {
		  	...
		  	columns: this.getColumns(),
				observeUser: this.getObserveUser()
		};
	...
	_isHelper: function(method, prop) {
		return !this._deniedRegex.test(prop) && !this._optionsRegex.test(prop) && this.isMethod(prop) && this._isFunction(method)
	},
	...
	getSubscription: function() {
		return this.subscriptions ? this.getSub() : this.pub;
	},
	getSelector: function() {
		//we hackily assign selector to subscription handle in ultimate_subscription/extend_ultimate_model.js
		if(this.getSub()) return this.getSub().selector;
		else return this._extract('selector');
	},
	getSub: function() {
		return UltimateModel._configObjToHandle(this.subscriptions[0]);
	},
	getObserveUser: function() {
		var sub = this.getSub();

		if(sub && sub.observeUser && sub.observeUser()) return {subName: sub.subName, class: sub.class};
		else return null;
	},
	...
	setupDatatableEvents: function() {
		_.chain(this.getPrototype())
			...
			.each(function(handler, name) {
				name = name.replace('on', '').capitalizeFirstLetter();
